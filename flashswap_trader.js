...
// define a function
function computeProfitMaximizingTrade(truePriceTokenA_, truePriceTokenB_, reserveA_, reserveB_) {
    let truePriceTokenA = BigInt(truePriceTokenA_);
    let truePriceTokenB = BigInt(truePriceTokenB_);
    let reserveA = BigInt(reserveA_);
    let reserveB = BigInt(reserveB_);
    // find the maximum profit 
    let aToB = reserveA * truePriceTokenB < truePriceTokenA * reserveB;
    let invariant = reserveA * reserveB;
    
    let leftSide = sqrt(
        invariant * BigInt(1000) * (aToB ? truePriceTokenA : truePriceTokenB) / 
        ((aToB ? truePriceTokenB : truePriceTokenA) * BigInt(997)));
    let rightSide = (aToB ? reserveA * BigInt(1000) : reserveB * BigInt(1000)) / BigInt(997);
    if (leftSide < rightSide) return {"AtoB" : false, "AmountIn": 0};

    // compute the amount that must be sent to move the price to the profit-maximizing price
    let amountIn = leftSide - rightSide;
    return {"AtoB" : true, "AmountIn" : amountIn}
}
...
let addrToken0 = process.env.ADDR_TOKEN0  // USDC
let addrToken1 = process.env.ADDR_TOKEN1  // ETH
...
newBlockEvent.on('data', async function(blockHeader){
    ...
    */
    ...
    try {
        ...
        //compute amount that must be traded to maximize the profit and, trade direction
        const result = computeProfitMaximizingTrade(sReserve0,sReserve1,uReserve0,uReserve1)   
        // end find the maximum profit
        const aToB = result['AtoB'] //trade direction
        const amountIn = parseInt(result['AmountIn'])

        if (amountIn==0) {console.log(`(0) No arbitrage opportunity on block ${blockHeader.number}\n`); return}
        ...
        if (aToB) { //T0->T1
            ...
            var difference =  amountOut/amountIn - 1/sPrice
            ...
            if (difference<=0) {console.log(`(1) No arbitrage opportunity on block ${blockHeader.number}\n`); return}

            //total difference (difference*quantity traded)
            //const totalDifference = difference*Math.round(amountIn/10**18)
            const totalDifference = Math.round(BigInt(difference)*BigInt(amountIn)/BigInt(10**18));
